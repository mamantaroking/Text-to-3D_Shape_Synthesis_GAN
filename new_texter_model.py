...
class Generator(nn.Module):
    def __init__(self, ngpu):
        ...
        self.main = nn.Sequential(
            ...
            nn.ConvTranspose3d(nz * 2, ngf * 3, 4, 1, 0, bias=False),
            nn.BatchNorm3d(ngf * 3),
            nn.ReLU(True),
            # state size. ''(ngf*8) x 4 x 4''
            nn.ConvTranspose3d(ngf * 3, ngf * 2, 4, 2, 1, bias=False),
            ...
            nn.ConvTranspose3d(ngf * 1, 32, 4, 2, 1, bias=False),
            nn.BatchNorm3d(32),
            nn.ReLU(True),
            # state size. ''(ngf) x 32 x 32''
            nn.ConvTranspose3d(32, nc, 4, 2, 1, bias=False),
            # nn.Tanh()
            nn.Sigmoid()
...
class Discriminator(nn.Module):
    def __init__(self, ngpu):
        ...
        self.main = nn.Sequential(
            ...
            nn.Conv3d(nc, 32, 4, 2, 1, bias=False),
            nn.LeakyReLU(0.2, inplace=True),
            # state size. ''(ndf) x 32 x 32''
            nn.Conv3d(32, ndf, 4, 2, 1, bias=False),
            nn.BatchNorm3d(ndf),
            nn.LeakyReLU(0.2, inplace=True),
        ...
        self.output = nn.Sequential(
            # nn.Conv3d(768, 1, 1, 1, 0, bias=False),
            # nn.Sigmoid()
            nn.Conv3d(nz + (ndf * 4), 1, 4, 1, 0, bias=False),
            nn.Sigmoid()
        )
